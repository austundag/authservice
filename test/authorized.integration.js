...
describe('user integration', function userIntegration() {
    ...
    it('error: get user without previous authentication', (done) => {
        superTester.get('/authorizeds/me', true, 401).end(done);
    });
    ...
    //    superTester.get('/authorizeds/me', jwt, 401).end(done);
    // });

    it('get super user', () => superTester.get('/authorizeds/me', true, 200)
        .then((res) => {
            const user = res.body;
    ...
    const getUserFn = function (index) {
        return function getUser() {
            ...
            return superTester.get(`/authorizeds/${id}`, true, 200)
                .then((res) => {
    ...
    it('list all non admin users', () => superTester.get('/authorizeds', true, 200)
        .then((res) => {
    ...
    const listUsersByRoleFn = function (role, range) {
        return function listUsersByRole() {
            return superTester.get('/authorizeds', true, 200, { role })
                .then((res) => {
    ...
    it('get new user', () => superTester.get('/authorizeds/me', true, 200)
        .then((res) => {
    ...
    it('error: create user with bad email', () => {
        ...
        return superTester.post('/authorizeds', userEmailErr, 400, undefined, true);
    });

    it('error: create the same user', () => {
        const user = hxUser.client(0);
        return superTester.post('/authorizeds', user, 400)
    ...
    it('error: create user with the same email', () => {
        ...
        return superTester.post('/authorizeds', newUser, 400)
    ...
    const verifySelfUserFn = function (index) {
        return function verifySelfUser() {
            return superTester.get('/authorizeds/me', true, 200)
                .then((res) => {
    ...
    const patchSelfUserEmailFn = function (index) {
        return function patchSelfUserEmail() {
            ...
            return superTester.patch('/authorizeds/me', { email }, 204)
                .then(() => {
                    const client = hxUser.client(index);
    ...
    const patchUserEmailFn = function (index) {
        return function patchUserEmail() {
            const { email } = authorizedGenerator.newUser();
            const id = hxUser.id(index);
            return superTester.patch(`/authorizeds/${id}`, { email }, 204)
                .then(() => {
                    const client = hxUser.client(index);
                    const server = hxUser.server(index);
                    if (!client.username) {
    ...
    it('update all user fields including password', () => superTester.patch('/authorizeds/me', userUpdate, 204));

    it('logout as new user', shared.logoutFn());

    it('error: bad login with old password', () => superTester.authBasic(hxUser.client(0), 401)
        .expect(() => {
    ...
    it('verify updated user fields', () => superTester.get('/authorizeds/me', true, 200)
        .then((res) => {
    ...
    const patchUserFn = function (index, userPatch) {
        return function patchUser() {
            const id = hxUser.id(index);
            return superTester.patch(`/authorizeds/${id}`, userPatch, 204)
                .then(() => {
                    const server = hxUser.server(index);
